...
Status DestroyList_Sq(SqList &L){       //销毁线性表L
    if(L.elem == NULL) return ERROR;    //确保顺序表存在
    free(L.elem);                       //释放顺序表内存
...
int LocateElem_Sq(SqList &L, ElemType e, Status (*compare)(ElemType, ElemType)){    //在顺序线性表中查找第1个值与e满足compare()的元素的位序,若找到，则返回其在L中的位序，否则返回0
...
Status PriorElem_Sq(SqList L, ElemType cur_e, ElemType &pre_e){ //若cur_e是L的数据元素，且不是第一个，则用pre_e返回它的前驱，否则操作失败，pre_e无定义
    ...
    if(i <= L.length) pre_e = L.elem[i - 2];
    else return ERROR;
    return OK; 
}

Status NextElem_Sq(SqList L, ElemType cur_e, ElemType &next_e){ //若cur_e是L的数据元素，且不是第一个，则用pre_e返回它的后继，否则操作失败，pre_e无定义
    if(L.elem == NULL || L.length < 2) return ERROR;
    int i = 1;                                          //从对歌位置开始查找
    while(i < L.length && L.elem[i - 1] != cur_e) i++;
...
void ListTraverse(SqList L, void (Visit)(ElemType)){    //依次对L的每个数据元素调用visit()
